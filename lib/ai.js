...
import { getCommitMessage } from "./github.js";

const DEFAULT_SYSTEM_PROMPT = `You are a talented technical writer who creates engaging development digests (weekly and daily).
Your style is:
...
- Visual — use emojis tastefully in headers, lists, or to add personality (e.g. 🚀 📦 ✨ 🔧). Use 1–3 per section where it fits.

Do not include images, image placeholders, or image suggestions — text and emojis only.

Output clean markdown. Use headers (##, ###), paragraphs, bullet points, code snippets, and the above when appropriate.`;
...
- Use emojis where they add personality (e.g. in section headers or key points).
...
#daily`;
  ...
  const userPrompt = `Write a short **daily** dev digest blog post for **${dayLabel}** (today's activity across all repos). Use the hashtag #daily (e.g. at the end of the post or after the title).

These repositories had activity today:

${repoContext}

Requirements:
- Title: something catchy and day-focused (e.g., "Today's Ship Log" or "What Shipped Today")
- Include the hashtag #daily in the post (not #weekly).
...
- Closing: one brief line, then #daily
- Keep it under ~300 words total
- Use markdown: ## for main headers, ### for subheaders, **bold** for emphasis
- Use emojis where they add personality (e.g. in headers or key points).
...
const TWEET_SYSTEM_PROMPT = `You write short tweets for a dev/product X (Twitter) account.
- Tone: playful yet professional — witty, approachable, no corporate fluff.
- Use 1–3 emojis per tweet where they fit naturally (e.g. 🚀 ✨ 🎲 🔧 💡).
- Max 280 characters total. No hashtags unless one is clearly fitting.
- Output only the tweet text, nothing else. No quotes, no "Tweet:" prefix.`;

/**
 * Generate a single tweet for a project: either project pitch or recent commit progress.
 * @param {object} opts
 * @param {string} opts.projectName - Display name (e.g. "BountyHub", "Dice Express")
 * @param {string} opts.projectDescription - Short description for context
 * @param {string} opts.projectUrl - Link to include (e.g. https://bountyhub.tech)
 * @param {string[]} [opts.recentCommitMessages] - Optional recent commit messages for "what we shipped" angle
 * @param {string} opts.apiKey - OpenAI API key
 * @returns {Promise<string>} Tweet text, max 280 chars
 */
export async function generateTweetForProject({
  projectName,
  projectDescription,
  projectUrl,
  ...
  const hasCommits = Array.isArray(recentCommitMessages) && recentCommitMessages.length > 0;
  const commitContext = hasCommits
    ? `Recent commit messages (use to highlight progress or ship vibes):\n${recentCommitMessages.slice(0, 5).map((m) => `- ${m}`).join("\n")}`
    : "No recent commits — focus on what the project is and why it's worth checking out.";

  const userPrompt = `Project: **${projectName}**
Description: ${projectDescription}
URL to include: ${projectUrl}

${commitContext}

Write one tweet that is either about recent progress (if commits given) or a short, catchy pitch for the project. Include the URL. Use emojis and a playful but professional tone. Output only the tweet, max 280 characters.`;
  ...
  if (!content) throw new Error("OpenAI returned empty tweet content");
  content = content.replace(/^["']|["']$/g, "").replace(/^Tweet:\s*/i, "").trim();
...
- Use markdown: ## for main headers, ### for subheaders, **bold** for emphasis
- Use emojis where they add personality.
- Tone: professional, smart, and lightly humorous.`;
