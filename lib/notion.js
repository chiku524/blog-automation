 ...
 * @param {string} [opts.feed] - Feed identifier (e.g. "generic" or "owner/repo") for database parent filtering
 * @param {string} [opts.theme] - Theme id (e.g. "weekly", "daily", "default"); sets page icon and optional Theme property
...
export async function createNotionPage({
  ...
  const themeConfig = theme ? getTheme(theme) : null;
  ...
  const properties = isDatabase
    ? {
        ...
        ...(feed && {
          Feed: {
            rich_text: [{ type: "text", text: { content: feed } }],
          },
        }),
      }
    : {
  ...
  const body = {
    parent,
    properties,
    ...(themeConfig && { icon: { type: "emoji", emoji: themeConfig.emoji } }),
